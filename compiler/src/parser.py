...
from src.tree.abstract_syntax_tree import *
from src.tree.json_parsed_tree import Node
...
class Parser:
    ...
    def parse(self):
        ...
        def expression_if(state, p):
            if self.syntax is True:
                return [Node("IF"), Node("("), Node("expression", p[2]), Node(")"), Node("{"), Node("block", p[5]),
                        Node("}")]
        ...
        def statement_assignment_type(state, p):
            ...
            return Assignment(Variable(p[1].getstr(), state, self.builder, self.module), p[3], state, self.builder,
                              self.module, type_=p[0].gettokentype())
        ...
        def statement_assignment(state, p):
            ...
            return Assignment(Variable(p[0].getstr(), state, self.builder, self.module), p[2], state, self.builder,
                              self.module, new=False)

        @self.pg.production('statement_full : FUNC INT IDENTIFIER ( args ) { block }')
        @self.pg.production('statement_full : FUNC FLT IDENTIFIER ( args ) { block }')
        def statement_func(state, p):
            if p[1].gettokentype() == 'INT':
                if self.syntax is True:
                    return [Node("FUNCTION"), Node('INT'), Node("IDENTIFIER", p[2]), Node("("), Node("args", p[4]),
                            Node(")"), Node("{"), Node("block", p[7]), Node("}")]
                return FunctionDeclaration(name=p[2].getstr(), typ=p[1].getstr(), args=p[4], block=p[7], state=state,
                                           builder=self.builder, module=self.module)
            else:
                if self.syntax is True:
                    return [Node("FUNCTION"), Node('FLT'), Node("IDENTIFIER", p[2]), Node("("), Node("args", p[4]),
                            Node(")"), Node("{"), Node("block", p[7]), Node("}")]
                return FunctionDeclaration(name=p[2].getstr(), typ=p[1].getstr(), args=p[4], block=p[7], state=state,
                                           builder=self.builder, module=self.module)
        ...
        @self.pg.production('args : arg , args')
        def args_expr_args(state, p):
            if self.syntax is True:
        ...
        @self.pg.production('expression : IDENTIFIER ( )')
        @self.pg.production('expression : IDENTIFIER ( args_call )')
        def expression_call(state, p):
            if p[2] == ')':
                if self.syntax is True:
                    return [Node("IDENTIFIER", p[0]), Node("("), Node("args_call", p[2]), Node(")")]
                return CallFunction(name=p[0].getstr(), args=None, state=state, builder=self.builder,
                                    module=self.module)
            else:
                if self.syntax is True:
                    return [Node("IDENTIFIER", p[0]), Node("("), Node("args_call", p[2]), Node(")")]
                return CallFunction(name=p[0].getstr(), args=p[2], state=state, builder=self.builder,
                                    module=self.module)
        ...
        @self.pg.production('args_call : expression , args_call')
        def args_expr_args(state, p):
            if self.syntax is True:
